...
def _clause_anchor(clause_ref: str) -> str:
    """
    Build a deterministic, PDF-safe anchor name for a clause reference.
    The same clause_ref must always produce the same anchor name so that
    a link in Critical Findings resolves to the matching clause table
    further down in the report.

    Examples:
      "46A-2"   -> "clause_46A_2"
      "F47A-3"  -> "clause_F47A_3"
      ""        -> ""
    """
    if not clause_ref:
        return ''
    s = re.sub(r'[^A-Za-z0-9]', '_', str(clause_ref).strip())
    return f'clause_{s}' if s else ''
...
def _build_executive_summary(
    ...
    if critical_findings:
        ...
        for i, cf in enumerate(critical_findings, 1):
            ...
            # check clause refs so the executive Critical Findings table
            # shows "Docs Check S26030280" rather than the raw synthetic
            # ref. Detail clause table further down keeps the same form.
            _cf_display_ref = _cf_clause_ref
            for _ps_pref in ('Presentation-', 'PartialShipment-'):
                if _cf_clause_ref.startswith(_ps_pref):
                    _cf_display_ref = f'Docs Check {_cf_clause_ref[len(_ps_pref):]}'
                    break
            # Wrap the clause reference in a hyperlink that jumps to the
            # matching clause table further down in the report.
            if _cf_anchor:
                _clause_cell = (
                    f'<font size="8"><b>'
                    f'<a href="#{_cf_anchor}" color="#1F3864">'
                    f'<u>{_esc(_cf_display_ref)}</u></a></b></font>'
                )
            else:
                _clause_cell = f'<font size="8"><b>{_esc(_cf_display_ref)}</b></font>'
            _cf_rows.append([
                Paragraph(f'<font size="8">{i}</font>', styles['CellTextSmall']),
                Paragraph(_clause_cell, styles['CellTextSmall']),
        ...
        if _review_display:
            ...
            for i, ri in enumerate(_review_display, 1):
                ...
                if _rv_anchor:
                    _rv_clause_cell = (
                        f'<font size="8"><b>'
                        f'<a href="#{_rv_anchor}" color="#1F3864">'
                        f'<u>{_esc(_rv_clause_ref)}</u></a></b></font>'
                    )
                else:
                    _rv_clause_cell = f'<font size="8"><b>{_esc(_rv_clause_ref)}</b></font>'
                _rv_rows.append([
                    Paragraph(f'<font size="8">{i}</font>', styles['CellTextSmall']),
                    Paragraph(_rv_clause_cell, styles['CellTextSmall']),
...
def _build_section_tables(sections: List[Dict], styles) -> List:
    ...
    for section in sections:
        ...
        for clause in section.get('clauses', []):
            ...
            # The <a name="..."/> embeds a named PDF destination so links from
            # the Critical Findings section can jump straight to this clause
            # table. The same anchor name is generated by _clause_anchor().
            _anchor_name = _clause_anchor(clause_ref)
            _anchor_tag = f'<a name="{_anchor_name}"/>' if _anchor_name else ''
            # P198gd — Special-case partial-shipment / presentation-check
            # synthetic clause refs so the navy bar reads like
            # "Documents Check — Invoice S26030280" instead of the raw
            # "LC Field: Presentation-S26030280". Same for the legacy
            # PartialShipment- prefix on existing data already on disk.
            _label_prefix = 'LC Field:'
            _label_value = clause_ref
            for _ps_pref in ('Presentation-', 'PartialShipment-'):
                if clause_ref.startswith(_ps_pref):
                    _label_prefix = 'Documents Check —'
                    _label_value = f'Invoice {clause_ref[len(_ps_pref):]}'
                    break
            _l1 = [[
                Paragraph(f'{_anchor_tag}<b><font color="white">{_label_prefix} {_esc(_label_value)}</font></b>',
                          styles['CellTextSmall']),
