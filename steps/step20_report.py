        ...
        for i, cf in enumerate(critical_findings[:30], 1):
            ...
            if _cf_anchor:
                _clause_cell = (
                    ...
                    f'<u>{_esc(_cf_clause_ref)}</u></a></b></font>'
                )
            else:
                _clause_cell = f'<font size="8"><b>{_esc(_cf_clause_ref)}</b></font>'
            _cf_rows.append([
...
def _build_section_tables(sections: List[Dict], styles) -> List:
    ...
    for section in sections:
        ...
        for clause in section.get('clauses', []):
            ...
            _l1 = [[
                Paragraph(f'{_anchor_tag}<b><font color="white">LC Field: {_esc(clause_ref)}</font></b>',
                          styles['CellTextSmall']),
